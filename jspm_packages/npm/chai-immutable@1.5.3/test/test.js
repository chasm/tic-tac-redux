...
describe('chai-immutable (' + typeEnv + ')', function() {
  ...
  describe('BDD interface', function() {
    ...
    describe('equal method', function() {
      ...
      it('should fail given a non-Immutable value', function() {
        fail(function() {
          expect([]).to.equal(List());
        });
      });
      it('should fail given different values', function() {
        fail(function() {
          expect(list3).to.equal(new List());
        });
      });
      it('should fail using `not` given equal values', function() {
        fail(function() {
          expect(list3).to.not.equal(List.of(1, 2, 3));
        });
      });
      it('should fail given deeply different values', function() {
        fail(function() {
          expect(deepMap).to.equal(differentDeepMap);
        });
        fail(function() {
          expect(deepMap).to.equals(differentDeepMap);
        });
        fail(function() {
          expect(deepMap).to.eq(differentDeepMap);
        });
        fail(function() {
          expect(deepMap).to.eql(differentDeepMap);
        });
        fail(function() {
          expect(deepMap).to.eqls(differentDeepMap);
        });
        fail(function() {
          expect(deepMap).to.deep.equal(differentDeepMap);
        });
      });
      it('should fail using `not` given deeply equal values', function() {
        fail(function() {
          expect(deepMap).to.not.equal(sameDeepMap);
        });
        fail(function() {
          expect(deepMap).to.not.equals(sameDeepMap);
        });
        fail(function() {
          expect(deepMap).to.not.eq(sameDeepMap);
        });
        fail(function() {
          expect(deepMap).to.not.eql(sameDeepMap);
        });
        fail(function() {
          expect(deepMap).to.not.eqls(sameDeepMap);
        });
        fail(function() {
          expect(deepMap).to.not.deep.equal(sameDeepMap);
        });
      });
    });
    describe('include method', function() {
      ...
      it('should fail given an inexisting value', function() {
        fail(function() {
          expect(new List([1, 2, 3])).to.include(42);
        });
      });
      it('should fail using `not` given an existing value', function() {
        fail(function() {
          expect(new List([1, 2, 3])).not.to.include(2);
        });
      });
    ...
    describe('keys method', function() {
      ...
      it('should accept a Map to check against', function() {
        expect(map).to.have.keys(new Map({
          'x': 6,
          'y': 7
        }));
      });
      ...
      it('should not affect the original assertions', function() {
        ...
        expect({
          x: 1,
          y: 2
        }).to.contain.any.keys('y', 'z');
        expect({
          x: 1,
          y: 2
        }).to.contain.any.keys(['x']);
        expect({
          x: 1,
          y: 2
        }).to.contain.any.keys({'x': 6});
        expect({
          x: 1,
          y: 2
        ...
        expect({
          ...
          y: 2,
          z: 3
        }).to.contain.all.keys(['x', 'y']);
        expect({
          x: 1,
          y: 2,
          z: 3
        }).to.contain.all.keys({'x': 6});
      });
      it('should display a helpful failure output on big objects', function() {
        var lengthyMap = new Map({foo: 'foo foo foo foo foo foo foo foo '});
        fail(function() {
      ...
      it('should fail given an inexisting key', function() {
        fail(function() {
          expect(new Map({x: 1})).to.have.key('z');
        });
        fail(function() {
          expect({x: 1}).to.have.key('z');
        });
      });
      it('should fail using `not` given an inexisting key', function() {
        fail(function() {
          expect(map).to.not.have.key('x');
        });
        fail(function() {
          expect(obj).to.not.have.key('x');
        });
      });
      it('should fail given multiple inexisting keys', function() {
        fail(function() {
          expect(map).to.have.keys('z1', 'z2');
        });
        fail(function() {
          expect(obj).to.have.keys('z1', 'z2');
        });
      });
      it('should fail using `not` given multiple existing keys', function() {
        fail(function() {
          expect(map).to.not.have.keys('x', 'y');
        });
        fail(function() {
          expect(obj).to.not.have.keys('x', 'y');
        });
      });
      it('should fail using `any` given inexisting keys', function() {
        fail(function() {
          expect(map).to.have.any.keys('z1', 'z2');
        });
        fail(function() {
          expect(obj).to.have.any.keys('z1', 'z2');
        });
      });
      it('should fail using `not` and `any` given an existing key', function() {
        fail(function() {
          expect(map).to.not.have.any.keys('x', 'z');
        });
        fail(function() {
          expect(obj).to.not.have.any.keys('x', 'z');
        });
      });
      it('should fail using `all` given an inexisting key', function() {
        fail(function() {
          expect(map).to.have.all.keys('z1', 'y');
        });
        fail(function() {
          expect(obj).to.have.all.keys('z1', 'y');
        });
      });
      it('should fail using `not` and `all` given existing keys', function() {
        fail(function() {
          expect(map).to.not.have.all.keys('x', 'y');
        });
        fail(function() {
          expect(obj).to.not.have.all.keys('x', 'y');
        });
      });
      it('should fail using `contain` given an inexisting key', function() {
        fail(function() {
          expect(map).to.contain.key('z');
        });
        fail(function() {
          expect(obj).to.contain.key('z');
        });
      });
    });
    describe('size method', function() {
      ...
      it('should fail given the wrong size', function() {
        fail(function() {
          expect(list3).to.have.size(42);
        });
      });
      it('should fail using `not` given the right size', function() {
        fail(function() {
          expect(list3).to.not.have.size(3);
        });
      });
    });
    describe('size property', function() {
      ...
      it('above should fail given a bad min size', function() {
        fail(function() {
          expect(list3).to.have.size.above(42);
        });
      });
      it('above should fail using `not` given a good min size', function() {
        fail(function() {
          expect(list3).to.not.have.size.above(2);
        });
      });
      ...
      it('below should fail given a bad max size', function() {
        fail(function() {
          expect(list3).to.have.size.below(1);
        });
      });
      it('below should fail using `not` given a good max size', function() {
        fail(function() {
          expect(list3).to.not.have.size.below(42);
        });
      });
      ...
      it('within should fail given a bad range', function() {
        fail(function() {
          expect(list3).to.have.size.within(10, 20);
        });
      });
      it('within should fail using `not` given a good range', function() {
        fail(function() {
          expect(list3).to.not.have.size.within(2, 42);
        });
      });
      ...
      it('least should fail given a bad min size', function() {
        fail(function() {
          expect(list3).to.have.size.of.at.least(42);
        });
      });
      it('least should fail using `not` given a good min size', function() {
        fail(function() {
          expect(list3).to.not.have.size.of.at.least(2);
        });
      });
      ...
      it('most should fail given a good max size', function() {
        fail(function() {
          expect(list3).to.have.size.of.at.most(2);
        });
      });
      it('most should fail using `not` given a bad max size', function() {
        fail(function() {
          expect(list3).to.not.have.size.of.at.most(42);
        });
      });
    });
  });
  describe('TDD interface', function() {
    describe('equal assertion', function() {
      ...
      it('should fail given different values', function() {
        fail(function() {
          assert.equal(list3, new List());
        });
      });
      it('should fail given deeply different values', function() {
        fail(function() {
          assert.equal(deepMap, differentDeepMap);
        });
      });
    });
    describe('notEqual assertion', function() {
      ...
      it('should fail given deeply equal values', function() {
        fail(function() {
          assert.notEqual(deepMap, sameDeepMap);
        });
      });
    });
    describe('unoverridden strictEqual and deepEqual assertions', function() {
      ...
      it('should fail given different values', function() {
        fail(function() {
          assert.strictEqual(list3, new List());
        });
        fail(function() {
          assert.deepEqual(list3, new List());
        });
      });
      it('should fail given deeply different values', function() {
        fail(function() {
          assert.strictEqual(deepMap, differentDeepMap);
        });
        fail(function() {
          assert.deepEqual(deepMap, differentDeepMap);
        });
      });
    });
    describe('unoverridden notStrictEqual and notDeepEqual assertions', function() {
      ...
      it('should fail given equal values', function() {
        fail(function() {
          assert.notStrictEqual(list3, List.of(1, 2, 3));
        });
        fail(function() {
          assert.notDeepEqual(list3, List.of(1, 2, 3));
        });
      });
      it('should fail given deeply equal values', function() {
        fail(function() {
          assert.notStrictEqual(deepMap, sameDeepMap);
        });
        fail(function() {
          assert.notDeepEqual(deepMap, sameDeepMap);
        });
      });
    });
