...
function NewExpression(node, parent) {
  ...
  if (node.arguments.length === 0 && this.format.compact && !t.isCallExpression(parent, {callee: node}) && !t.isMemberExpression(parent) && !t.isNewExpression(parent))
    return;
  this.push("(");
...
function AssignmentExpression(node, parent) {
  ...
  if (!spaces) {
    ...
    }) || t.isUpdateExpression(node.right, {
      prefix: true,
      operator: node.operator + node.operator
    }) || t.isBinaryExpression(node.right) && t.isUnaryExpression(getLeftMost(node.right), {
      prefix: true,
      operator: node.operator
    });
  }
